...
grep -q 'applied=36' /tmp/mujeeb24-migrate-schema-test-1.log
grep -q 'applied=0' /tmp/mujeeb24-migrate-schema-test-2.log
[[ "$(${DOCKER[@]} exec "$CONTAINER" psql -p "$PORT" -U postgres -d "$RUNNER_DB" -Atqc 'SELECT count(*) FROM schema_migrations')" == "36" ]]

echo 'postgres_schema_validation=passed'
echo 'foundation_constraints=passed'
echo 'full_schema_constraints=passed'
echo 'migration_runner_first=applied-36'
