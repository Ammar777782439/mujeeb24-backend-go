...
func (r *countingRuntime) Decide(_ context.Context, _ ports.AIDecisionInput) (ports.AIDecisionProposal, error) {
	r.calls++
	return r.proposal, nil
}

func TestAutoReply_OneAICallPerTurn(t *testing.T) {
	rt := &countingRuntime{proposal: ports.AIDecisionProposal{
		IntentBase: "information_request", RequestedAction: "no_action",
		ConfidenceBand: "high", PolicyDecision: "allowed", SchemaVersion: 1,
		StateProposal: &ports.AIStateProposal{Kind: "NO_REFERENCE"},
	}}
	...
	if rt.calls != 1 {
		t.Fatalf("expected 1 AI call, got %d", rt.calls)
	}
}
...
func (s stubRefRepo) GetByID(_ context.Context, _, _ string) (ports.ConversationReferenceRecord, error) {
	return ports.ConversationReferenceRecord{}, nil
}
