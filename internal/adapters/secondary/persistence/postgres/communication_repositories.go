...
func (r *OutboundMessageRepository) CreatePending(ctx context.Context, draft ports.OutboundMessageDraft) (ports.OutboundMessageRecord, error) {
	...
	const query = `INSERT INTO outbound_messages (id, business_id, conversation_id, conversation_reference_id, connection_id, provider_ref, channel, origin, direction, transport, content_reference, provider_idempotency_key, status, correlation_id, causation_id, created_at, updated_at) VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid, $6, $7, $8, 'outbound', $9, $10, $11, 'pending', $12::uuid, $13::uuid, now(), now()) RETURNING id::text, business_id::text, conversation_id::text, conversation_reference_id::text, connection_id::text, provider_ref, channel, origin, direction, transport, content_reference, provider_idempotency_key, status, provider_message_id, failure_code, attempt_count, correlation_id::text, causation_id::text`
...
func (r *OutboundMessageRepository) GetByID(ctx context.Context, businessID, messageID string) (ports.OutboundMessageRecord, error) {
	...
	const query = `SELECT id::text, business_id::text, conversation_id::text, conversation_reference_id::text, connection_id::text, provider_ref, channel, origin, direction, transport, content_reference, provider_idempotency_key, status, provider_message_id, failure_code, attempt_count, correlation_id::text, causation_id::text FROM outbound_messages WHERE business_id = $1::uuid AND id = $2::uuid`
...
func (r *OutboundMessageRepository) MarkProviderAccepted(ctx context.Context, businessID, messageID, providerMessageID string) (ports.OutboundMessageRecord, error) {
	...
	const query = `
		...
		RETURNING id::text, business_id::text, conversation_id::text, conversation_reference_id::text, connection_id::text, provider_ref, channel, origin, direction, transport, content_reference, provider_idempotency_key, status, provider_message_id, failure_code, attempt_count, correlation_id::text, causation_id::text`
