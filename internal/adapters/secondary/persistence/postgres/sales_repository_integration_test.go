...
func TestSalesRepositoriesAgainstPostgres(t *testing.T) {
	...
	_, _ = pool.Exec(ctx, `DELETE FROM order_lines WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM transaction_reviews WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM transaction_confirmations WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM commercial_transactions WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM lead_scores WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM lead_attributions WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM leads WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM offers WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM variants WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM catalog_items WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
	_, _ = pool.Exec(ctx, `DELETE FROM catalogs WHERE business_id IN ($1::uuid, $2::uuid)`, businessA, businessB)
