...
func TestAutoReplyContextBuilderGroundsCatalogAgainstPostgres(t *testing.T) {
	...
	if _, err := adapter.Pool().Exec(ctx, `INSERT INTO channel_connections (id, business_id, provider_ref, channel, provider_account_ref, provider_connection_ref, status, secret_reference, created_at, updated_at) VALUES ($1::uuid, $2::uuid, 'socialapi', 'whatsapp', 'account-1', 'connection-1', 'active', 'local-secret-ref', $3, $3)`, connectionID, businessID, base); err != nil {
	...
	if contextValue.KnowledgeState != services.AIContextPartial || contextValue.Freshness != services.AIContextFresh {
