...
func (r *MerchantAISessionRepository) CreateSession(ctx context.Context, session ports.MerchantAISessionRecord) (ports.MerchantAISessionRecord, error) {
	if r == nil || r.adapter == nil {
		return ports.MerchantAISessionRecord{}, ErrPoolClosed
	}
	executor, err := r.adapter.Executor(ctx)
	if err != nil {
		return ports.MerchantAISessionRecord{}, err
	}
	if strings.TrimSpace(session.ID) == "" || strings.TrimSpace(session.BusinessID) == "" || strings.TrimSpace(session.PrincipalID) == "" {
	...
	const query = `INSERT INTO merchant_ai_sessions (id, business_id, principal_id, created_at, updated_at)
		...
		RETURNING id::text, business_id::text, principal_id::text, created_at, updated_at`
	var record ports.MerchantAISessionRecord
	err = executor.QueryRow(ctx, query, session.ID, session.BusinessID, session.PrincipalID, now, updatedAt).
		Scan(&record.ID, &record.BusinessID, &record.PrincipalID, &record.CreatedAt, &record.UpdatedAt)
	if err != nil {
		return ports.MerchantAISessionRecord{}, classifyRepositoryWriteError("merchant_ai_session.create", err)
	}
	return record, nil
}
...
func (r *MerchantAISessionRepository) GetSession(ctx context.Context, businessID, principalID, sessionID string) (ports.MerchantAISessionRecord, error) {
	...
	if strings.TrimSpace(businessID) == "" || strings.TrimSpace(principalID) == "" || strings.TrimSpace(sessionID) == "" {
	...
	const query = `SELECT id::text, business_id::text, principal_id::text, created_at, updated_at
		FROM merchant_ai_sessions
		WHERE id = $1::uuid AND business_id = $2::uuid AND principal_id = $3::uuid`
	var record ports.MerchantAISessionRecord
	err = executor.QueryRow(ctx, query, sessionID, businessID, principalID).
		Scan(&record.ID, &record.BusinessID, &record.PrincipalID, &record.CreatedAt, &record.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return ports.MerchantAISessionRecord{}, &RepositoryError{Operation: "merchant_ai_session.get", Kind: RepositoryNotFound, Err: err}
		}
...
func (r *MerchantAISessionRepository) AppendMessage(ctx context.Context, draft ports.MerchantAIMessageDraft) (ports.MerchantAIMessageRecord, error) {
	if r == nil || r.adapter == nil {
		return ports.MerchantAIMessageRecord{}, ErrPoolClosed
	}
	executor, err := r.adapter.Executor(ctx)
	if err != nil {
		return ports.MerchantAIMessageRecord{}, err
	}
	if strings.TrimSpace(draft.ID) == "" || strings.TrimSpace(draft.BusinessID) == "" || strings.TrimSpace(draft.SessionID) == "" || strings.TrimSpace(draft.Text) == "" {
	...
	senderType := strings.TrimSpace(draft.SenderType)
	...
	const query = `INSERT INTO merchant_ai_messages (id, business_id, session_id, sender_type, text, created_at)
		...
		RETURNING id::text, business_id::text, session_id::text, sender_type, text, created_at`
	var record ports.MerchantAIMessageRecord
	err = executor.QueryRow(ctx, query, draft.ID, draft.BusinessID, draft.SessionID, senderType, draft.Text, now).
		Scan(&record.ID, &record.BusinessID, &record.SessionID, &record.SenderType, &record.Text, &record.CreatedAt)
	if err != nil {
		return ports.MerchantAIMessageRecord{}, classifyRepositoryWriteError("merchant_ai_message.append", err)
	}
	return record, nil
}
...
func (r *MerchantAISessionRepository) ListRecentMessages(ctx context.Context, businessID, sessionID string, limit int) ([]ports.MerchantAIMessageRecord, error) {
	...
	const query = `SELECT id::text, business_id::text, session_id::text, sender_type, text, created_at
		FROM (
			SELECT id, business_id, session_id, sender_type, text, created_at
	...
	defer rows.Close()
	var records []ports.MerchantAIMessageRecord
	for rows.Next() {
...
func (r *MerchantAISessionRepository) TouchSession(ctx context.Context, businessID, sessionID string, updatedAt time.Time) error {
	...
	tag, err := executor.Exec(ctx, query, now, sessionID, businessID)
	if err != nil {
