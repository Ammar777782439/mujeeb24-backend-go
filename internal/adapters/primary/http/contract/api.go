...
func registerExtendedOperations(api huma.API, dispatcher DashboardOperationHandler) {
	...
	register(api, dispatcher, huma.Operation{OperationID: "chatWithMerchantAI", Method: http.MethodPost, Path: "/businesses/{business_id}/ai/chat", Tags: []string{"AI"}, Summary: "Send a message to the Merchant AI assistant", Security: dashboardSecurity, DefaultStatus: http.StatusOK, Errors: []int{http.StatusBadRequest, http.StatusUnauthorized, http.StatusForbidden, http.StatusNotFound, http.StatusConflict, http.StatusUnprocessableEntity, http.StatusServiceUnavailable}}, MerchantAIChatInput{}, Single[MerchantAIChatResponse]{})
